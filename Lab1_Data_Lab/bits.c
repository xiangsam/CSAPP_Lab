...
int isTmax(int x) {
  int tmp = (x + 1) ^ x;
  return (!(tmp+1)) & (!!(x+1)); //0xffffffff or 0x7fffffff
}
/* 
...
int isAsciiDigit(int x) {
  int hi = !((x>>4) ^ 0x3);
  int lo = !((x&0x8)>>3) | !(x&0x6); // 0xxx or 100x is ok
  return hi & lo;
}
/* 
...
int floatFloat2Int(unsigned uf) {
  ...
  if(E < 0) return 0; // M < 2
  if(E > 23) value = frac<<(E-23);
  else if(E<= 23) value = frac >> (23-E);
